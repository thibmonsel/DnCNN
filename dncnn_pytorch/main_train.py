...
from utils import sum_squared_error, log, divergence_loss,divergence_loss2
...
def train_sure(model, device, train_loader, optimizer, eps, sigma, epoch): 
    model.train()
    
    running_train_loss = 0
    for step, data in enumerate(train_loader):
        optimizer.zero_grad()
        noisy_image, _ = data[0].to(device), data[1].to(device)
        residual_image = model(noisy_image)        
        
        n = torch.normal(0., 1, size = tuple(noisy_image.shape))  
        n = n.to(device)
        noisy_image_gauss = noisy_image + eps * n
        
        noisy_image_gauss = noisy_image_gauss.to(device)
        out_noisy_image_gauss = model(noisy_image_gauss)

        # part1_loss = nn.MSELoss(reduction='sum')(noisy_image, residual_image)/2
        part1_loss = nn.MSELoss()(noisy_image, residual_image)
        part2_loss = divergence_loss2(residual_image, out_noisy_image_gauss, n, eps, sigma)
        loss = part1_loss + 0.01 * part2_loss - sigma ** 2
...
if __name__ == '__main__':
    ...
    for epoch in range(n_epoch):
        ...
        loss = train_sure(model, device, DLoader, optimizer, eps, sigma, epoch)
        # loss = train(model, device, DLoader, criterion, optimizer, epoch)  
        scheduler.step(epoch)
      
        torch.save(model, os.path.join(save_dir, 'sure_model_%03d_training_mod.pth' % (epoch+1)))
        
    torch.save(model, os.path.join(save_dir, 'sure_final_model.pth'))
